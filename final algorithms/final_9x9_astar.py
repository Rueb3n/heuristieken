...
# aantal nodes bewogen van dit bord + (afstand rode auto naar + autos voor de rode auto)

#    f(p)=cost(p)+h(p)    
def a_star(moved_nodes, vehicle_dict, child_df_grid):
	blocking_cars = 0
	...
	# print moved_nodes
	total_cost = moved_nodes + distance_to_goal + blocking_cars
	print total_cost
	return total_cost
...
def start_bfs_astar(start_grid):
	z = 0
	find_path = dict()
	vehicle_dict = dict()
	archive = deque()
	...
	while heap_queue:
		# print '\N\N\N\N FINISHED \N\N\N\N', finished
		print '\n HELLO BEFORE \n'
		...
		# if z == 10:
		# 	sys.exit()
		heap_archive.append(df_grid)
		heap_queue = heap_queue[:len(heap_queue) * 3 // 5]
		for key, value in vehicle_dict.iteritems():
			...
			while movelist:
				...
				if not child_df_grid in heap_archive:
					...
					cost_child_astar = a_star(board_count_dict.get(str(child_df_grid)),new_vehicle_dict,pd.DataFrame(child_df_grid))
					# queue.put((cost_child_astar, child_df_grid))
					heapq.heappush(heap_queue,(cost_child_astar, child_df_grid))


def start_bfs_heuristic(start_grid):
	z = 0
	find_path = dict()
	vehicle_dict = dict()
	archive = deque()
	...
	while heap_queue:
		...
		# 	sys.exit()
		heap_archive.append(df_grid)
		heap_queue = heap_queue[:len(heap_queue) * 1 // 5]
		for key, value in vehicle_dict.iteritems():
			# print key, value
			check_adjecent(key, value, vehicle_dict)
			movelist, object_location, direction = get_movelist(key, value, vehicle_dict, pd.DataFrame(df_grid))
...
def main():
	start_bfs_astar(grid)
	# start_bfs_heuristic(grid)
